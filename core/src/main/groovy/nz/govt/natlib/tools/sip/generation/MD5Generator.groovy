...
class MD5Generator {
    ...
    static String generateOrReadMD5HashFile(File sourceFile, boolean overwriteIfAlreadyExists) throws SipGenerationException {
        String md5Hash
    ...
    static String readMD5Hash(File md5SourceFile) throws SipGenerationException {
        String md5Hash
    ...
    static String calculateMd5Hash(File file) throws SipGenerationException {
    ...
    static boolean isValidSourceFile(File file) throws SipGenerationException {
        if (!file.exists()) {
            throw new SipGenerationException("File=${file.getAbsolutePath()} does not exist. Cannot generate MD5 from file.")
        }
        if (!file.isFile()) {
            throw new SipGenerationException("File=${file.getAbsolutePath()} is not a file. Cannot generate MD5 from file.")
        }
        return true
    }

    static File determineMd5File(File sourceFile) throws SipGenerationException {
